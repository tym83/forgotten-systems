...
uint32_t risc_get_pc(struct RISC *risc) { return risc->PC; }
...
uint32_t risc_get_flags(struct RISC *risc) {
  return ((uint32_t)risc->N << 3) | ((uint32_t)risc->Z << 2)
       | ((uint32_t)risc->C << 1) | (uint32_t)risc->V;
}
